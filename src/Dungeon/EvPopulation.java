...
public class EvPopulation 
{
	 ...
	 */
	public ArrayList<Dungeon> populationInitialization(int f, int c, int numero_poblacion, int numero_monstruos, int numero_tesoros, int numero_puertas )
	{
				...
				for(int columna = 0; columna < c; columna++)
				{
					...
					for(int elemento = 0; elemento < lista.size(); elemento++)
					{
						...
						for (int num_dir_opuesta = 0; num_dir_opuesta < lista_opuesta.size(); num_dir_opuesta++)
						{
							...
							if(lista_opuesta.get(num_dir_opuesta).direction == opuesta && random == 1)
							{
