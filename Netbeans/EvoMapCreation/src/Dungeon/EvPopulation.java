...
public class EvPopulation {
    ...
    /**
     * Funcion que se encarga de inicializar una poblacion con (x,y)
     * dimensiones, x monstruos, x tesoros, x puertas y paredes aleatorias
     *
...
//			System.out.println("Indiv: " + i);
    ...
    /**
     * Funcion que selecciona 2 individuos de la poblacion con el mejor fitness
     * entra un grupo de n individuos
     *
     * @param Poblacion_ Poblacion de la que se va a seleccionar a los
     * individuos
...
//			}
    ...
    /**
     * Funcion que va a reemplazar el mejor individuo de la nueva generacion con
     * el peor individuo de los individuos seleccionados como peor generacion
     *
     * @param Poblacion_ Poblacion que se quiere modificar
     * @param newGeneration Individuos que se han seleccionado de cara a mejorar
     * la poblacion (solo se pondra el mejor de los 2)
     * @return Devuelve la poblacion modificada con el mejor de la nueva
     * generacion sustituido por el peor entre los seleccionados
     * @throws CloneNotSupportedException
     */
            ...
            for (int individuo = 0; individuo < Poblacion_.size(); individuo++) {
                ...
                if (i == 0) {
                    ...
                    if (individuo == 0) {
                        ...
                        if (Poblacion_.get(posicion_peores[i]).fitness < Poblacion_.get(individuo).fitness) {
                            //guardo la posicion en la poblacion del peor individuo
                ...
                //comprobamos que el que vamos a seleccionar no sea ninguno de los ya elegidos
                    ...
                    if (individuo == 0) {
                        ...
                        for (int n_pos = 0; n_pos < posicion_peores.length; n_pos++) {
                            if (posicion_peores[n_pos] == 0) {
                                exists = true;
                            }
                        }

                        //si el peor individuo guardado es la posición 0 guardamos la siguiente posicion
                        if (exists) {
                            //guardo la posicion en la poblacion del peor individuo
                            posicion_peores[i] = 1;
                        } else {
    ...
    /**
     * Funcion que si el numero random que genera esta entre el 5%, muta
     * aleatoriamente un gen de un individuo aleatorio de la poblacion
     *
     * @param Poblacion_ Poblacion que se va a mutar
     * @return Devuelve la poblacion con la mutacion en el individuo
     * correspondiente
     * @throws CloneNotSupportedException
     */
        ...
        if (random_num < 0.10)// si sale un porcentaje de un 5% modificamos uno de los individuos de la poblacion
        {
            ...
            if (individuo_mutado.fitness == -100) {
                ...
                if (mejorIndivSelected) {
                    Poblacion_noValidos.add((Dungeon) individuo_mutado.clone());
                    Poblacion_.remove(Poblacion_.size() - 1);
                } else {
                    //se anade a la poblacion de no validos el individuo
                    Poblacion_noValidos.add((Dungeon) Poblacion_.get(random_individuo_mutacion).clone());
...
//			individuo2.calcularfitness(individuo2.numero_puertas);
    ...
    /**
     * Funcion que comprueba si esta evolucionando la poblacion o esta
     * empeorando, si es asi, para la ejecucion
     *
     * @param Poblacion_ Poblacion a comprobar para ver si esta mejorando
     * @return Devuelve true si no evoluciona mas la poblacion o ha encontrado
     * al individuo idoneo
     */
...
//		System.out.print("\n");
    ...
    /**
     * Funcion que calcula la desviacion tipica de los individuos de la
     * poblacion
     *
     * @return
     */
    ...
    /**
     * Funcion que calcula el intervalo de confianza de los individuos de la
     * poblacion
     *
     ...
     *
     * @param Individuo Posicion en la que se encuentra el individuo en la
     * poblacion
