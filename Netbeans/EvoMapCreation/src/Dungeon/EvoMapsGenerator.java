...
import Dungeon.EvPopulation;
import Dungeon.LogWriter;
...
public class EvoMapsGenerator
{
    ...
    int porcentaje_mons = 9;
    int porcentaje_tes = 6;
    public int numero_monstruos = (int)((porcentaje * porcentaje_mons)/100 );
    public int numero_tesoros = (int)((porcentaje * porcentaje_tes)/100 );
...
//		System.setOut(out);
        ...
        evopopulation.max_iter = 500;

        ArrayList<Dungeon> Poblacion_provisional;

        while (Poblacion.size() < numero_poblacion) 
        {
            // Inicializo la
            // poblacion*********************************************************************
...
//				
                ...
                if (Poblacion.size() >= 2)
                {
                    ...
                    Poblacion = (ArrayList<Dungeon>) evopopulation.mutation((ArrayList<Dungeon>) Poblacion.clone()).clone();
                    //evopopulation.Poblacion = Poblacion;
                }
...
//				
                ...
                if (Poblacion.size() < numero_poblacion){
                    while(Poblacion.size() < numero_poblacion){
                        ArrayList<Dungeon> nuevoIndiv = new ArrayList<Dungeon>();
                        nuevoIndiv = evopopulation.populationInitialization(f, c, 1, numero_monstruos, numero_tesoros, pos_puertas, t_puertas, numero_puertas, porcentaje, porcentaje_paredes, tipo_celdas, dificultad_nivel, ponderaciones_nivel);
                        
                        if(nuevoIndiv.get(0).dungeon_valido && nuevoIndiv.get(0).fitness >= 0){
                            Poblacion.add((Dungeon) nuevoIndiv.get(0).clone());
                        }
                    }
                ...
                if (Poblacion.size() > numero_poblacion){
                    while(Poblacion.size() > numero_poblacion){
                        int worstPos = -1; 
                        worstPos = evopopulation.getWorstIndividuoPos((ArrayList<Dungeon>) Poblacion.clone());
                        this.Poblacion_No_Validos.add((Dungeon) Poblacion.get(worstPos).clone());
                        Poblacion.remove(worstPos);
