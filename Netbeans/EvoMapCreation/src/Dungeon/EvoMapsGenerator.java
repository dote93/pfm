...
//				System.out.print("\n");
                ...
                /**
                 * Antes de seleccionar a cualquier individuo, primero se
                 * transladan los individuos con fitness -100 a la poblacion de
                 * no validos*
                 */
                for (int i = 0; i < Poblacion.size(); i++) {
...
//                while (Poblacion.size() < numero_poblacion) {
//                for(int nuevo_ind= 0; nuevo_ind< 2; nuevo_ind++){
                    Poblacion_provisional = new ArrayList<Dungeon>();
                    
                    while(Poblacion_provisional.size() != 2){
                        ArrayList<Dungeon> nuevos_Indiv = evopopulation.populationInitialization(f, c, 1, numero_monstruos, numero_tesoros, pos_puertas, t_puertas, numero_puertas, porcentaje, porcentaje_paredes, tipo_celdas, dificultad_nivel, ponderaciones_nivel);

                        for (int i = 0; i < nuevos_Indiv.size(); i++) {
                            if (nuevos_Indiv.get(i).dungeon_valido && nuevos_Indiv.get(i).fitness >= 0.0) {
                                Poblacion_provisional.add((Dungeon) nuevos_Indiv.get(i).clone());
                            }
                    ...
                    for(int nuevo_ind= 0; nuevo_ind< 2; nuevo_ind++){
                        Poblacion.add((Dungeon) Poblacion_provisional.get(nuevo_ind).clone());
                    }
